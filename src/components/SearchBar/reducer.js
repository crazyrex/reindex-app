...
function updateCategoriesValue(search, data) {
  ...
  if (!data.searchText) {
    ...
    if (data.first) tmp.filtersValue[data.filter] = [];
      ...
      tmp.filtersValue[data.filter] = [...spliceArray(tmp.filtersValue[data.filter], data.index), data.searchText];
    }
    ...
    case SET_ACTIVE_TAB:
      return Object.assign({}, state, {
        search: Object.assign({}, state.search, {
          ...
          [action.activeTab]: Object.assign({}, state.search[action.activeTab], {
            value: (action.activeTab === 'people') ? state.search.businesses.value : state.search.people.value
          }),
        }),
      });
