import React from 'react';
import proj4 from 'proj4';
require('./Mapbox.scss');


class GovMap extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      location: {},
    };
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.data && this.props.data.length){
      
      let data = nextProps.data;
      console.log('ppppp',data)
      var firstProjection = 'PROJCS["NAD83 / Massachusetts Mainland",GEOGCS["NAD83",DATUM["North_American_Datum_1983",SPHEROID["GRS 1980",6378137,298.257222101,AUTHORITY["EPSG","7019"]],AUTHORITY["EPSG","6269"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.01745329251994328,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4269"]],UNIT["metre",1,AUTHORITY["EPSG","9001"]],PROJECTION["Lambert_Conformal_Conic_2SP"],PARAMETER["standard_parallel_1",42.68333333333333],PARAMETER["standard_parallel_2",41.71666666666667],PARAMETER["latitude_of_origin",41],PARAMETER["central_meridian",-71.5],PARAMETER["false_easting",200000],PARAMETER["false_northing",750000],AUTHORITY["EPSG","26986"],AXIS["X",EAST],AXIS["Y",NORTH]]';
      var secondProjection = "+proj=gnom +lat_0=90 +lon_0=0 +x_0=6300000 +y_0=6300000 +ellps=WGS84 +datum=WGS84 +units=m +no_defs";
       console.log('protytj4',proj4(firstProjection,[186587.47,659471.41]));
      // if (nextProps.location.latitude && nextProps.location.longitude)
         govmap.createMap('map', {  
               token:'aaaaa',  
               center: { x: 32.0923988, y: 34.839107399999996 } 
         });  

        govmap.zoomToXY({ q:'רחוב0ההגנה,%20אור%20יהודה', level: 10, marker: true });
        let  params={  
           "keyword":"ההגנה", 
           "type":govmap.geocodeType.AccuracyOnly     
           };  
        console.log('rtttuyt', govmap.geocode(params)   )
        govmap.geocode( params)  
              .then(function (result){ 
                  console.log('tttttt',result)
              .error(function(err){
                  console.log('eeee',err)
              });
                  
         });  
        // let convert ,x ,y;

        //  for(let i = 0; i< data.length; i++) {
        //    if (data[i]._source.location && data[i]._source.location[0] && data[i]._source.location[1]){
        //     console.log('yyyyyyyyyy');
        //     convert = proj4(secondProjection,[data[i]._source.location[0],data[i]._source.location[1]]);
        //     console.log('aaaaaa',convert)
        //     x = convert[0].toFixed();
        //     y = convert[1].toFixed();
        //     console.log('ooooo',x ,y )
        //     govmap.zoomToXY({ x: x, y: y, level: 10, marker: true });
        //    }
         
        //  }
    }

  }
   
 

  render() {

    return (
      <section className="sec">
       <div id="map"  style={{
          position: 'absolute',
          top: 0,
          bottom: 0,
          width: '100%',
          height: '100%',
          }}></div>
      </section>
    );
  }
}

GovMap.propTypes = {
  location: React.PropTypes.object,
};

export default GovMap;