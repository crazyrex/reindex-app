...
class Mapbox extends React.PureComponent {
  ...
  componentDidMount(nextProps) {
    if (this.props.location && this.props.location.location)
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i]._source.reindexLocationPoints && data[i]._source.reindexLocationPoints.length) {

          var el = document.createElement('div');
          el.className = 'marker';
          var marker = new mapboxgl.Marker(el)
            .setLngLat(data[i]._source.reindexLocationPoints)
            .addTo(map);
          var markerHeight = 30,
            markerRadius = 10,
            linearOffset = 25;
          ...
          var popup = new mapboxgl.Popup({
            ...
            .setLngLat(data[i]._source.reindexLocationPoints)
            .setHTML(`<h5>${data[i]._source.reindexTitle}</h5>`)
            .addTo(map);
        }
      }
    }
  ...
  changeMarkerColor(index) {
    if (index === this.props.data.length - 1) index = 0;
    this.setState({
      currentIndex: index
    });
  ...
  render() {
    return (
      <section className="sec">
        {this.props.location ? <div id="mapbox1"></div> :
          <div
            id="mapbox"
        ...
        {this.props.data.map((record, key) =>
          <Slide
            key={key}
            media={<div></div>}
            title={record._source.reindexTitle}
            contentStyle={{ backgroundColor: 'white' }}
            subtitle={record._source.reindexLocationString}
          />
        )}
        </AutoRotatingCarousel>
      </section>
